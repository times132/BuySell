    ...
    function init() {
        ...
        chatService.findAllRoom(function (data) {
            ...
            for (var i = 0, len = data.length || 0; i < len; i++) {
                ...
                if(data[i].users.length == 1) { // 채팅방에 혼자남았을 때
...
<script>
    ...
    function initMessage() {
        ...
        chatService.findAllMessages(roomId, function (data) {
            ...
            for (var i = 0, len = data.length || 0; i < len; i++) { // 대화 내용이 있을 때
                appendMsg(data[i]);
            }
    ...
    function appendMsg(data){
        ...
        if (data.type == "BOARD"){  // 게시판내용인 경우
            ...
            str +=
                ...
                "</div>"+
                "</div></div></div>";
        } else if(senderId == data.senderId) { // 내가 쓴 메시지는 오른쪽
